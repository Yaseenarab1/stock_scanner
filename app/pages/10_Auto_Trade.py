...
tab_dash, tab_settings, tab_creds, tab_positions, tab_help = st.tabs(
    ["📊 Dashboard", "⚙️ Settings", "🔐 Credentials", "📂 Positions & History", "📘 Help"]
)
...
with tab_dash:
    ...
    kpi_row([
        {"label": "Auto-trade", "value": ("🟢 ON" if enabled else "🔴 OFF"),
         "trend": "up" if enabled else "down"},
        {"label": "Trades today", "value": f"{tdone} / {maxd}"},
        {"label": "Last trade date", "value": (str(ltd) if ltd else "—")},
        {"label": "Credentials",
         "value": ("✅ On file" if (has_key and has_secret) else "⚠️ Missing"),
         "trend": "up" if (has_key and has_secret) else "down"},
    ])
    ...
    if open_pos:
        ...
        section("Live Position Controls")
        st.caption(
...
with tab_creds:
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    st.divider()
    section("Webull API Credentials")
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    st.divider()
    section("Test Webull Connection")
    st.caption(
...
with tab_positions:
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    st.divider()
    section("Today's Stop-Loss Blacklist")
    st.caption(
    ...
    if not bl_rows:
        ...
        st.dataframe(bl_df, use_container_width=True, hide_index=True,
                     column_config=auto_column_config(bl_df))
