                ...
                with pg() as con:
                    for s in snaps:
                        ...
                        if info["baseline_8am"] is None:
                            fetched = fetch_exact_8am_baseline(ticker, now_et)
                            if fetched and fetched > 0:
                                info["baseline_8am"] = fetched
                            # else: leave baseline None so the NEXT cycle retries.
                            # Do NOT pin it to the current mid-window price — that
                            # permanently anchors gain near 0 and locks the ticker
                            # out of the 10% qualify gate for the rest of the day.

                        # Working baseline for THIS cycle's gain math only. If the
                        # baseline hasn't resolved yet we use the current price
                        # (gain ~0 this cycle), but we do not persist that as the
                        # baseline, so a later cycle can still anchor it correctly.
